# ============================================================================
# MODULE 06: GRASSLAND CARBON STOCK AGGREGATION & AAFC/Canadian Protocol COMPLIANCE
# ============================================================================
# PURPOSE: Aggregate carbon stocks from Modules 04/05, calculate conservative
#          estimates, compare methods, and generate AAFC/Canadian Protocol compliance reports
# INPUTS:
...
log_message("=== MODULE 06: CARBON STOCK CALCULATION & AAFC/Canadian Protocol COMPLIANCE ===")
...
# AAFC/Canadian Protocol depth intervals are loaded from grassland_carbon_config.R
# AAFC/Canadian Protocol_DEPTH_INTERVALS data frame has: depth_top, depth_bottom, depth_midpoint, thickness_cm

# IPCC/AAFC/Canadian Protocol Confidence Level
CONFIDENCE_LEVEL <- 0.95  # 95% CI required by AAFC/Canadian Protocol
...
log_message(sprintf("AAFC/Canadian Protocol depth intervals: %d layers", nrow(AAFC/Canadian Protocol_DEPTH_INTERVALS)))
...
# ============================================================================
# AGGREGATE STOCKS BY AAFC/Canadian Protocol DEPTH INTERVALS
# ============================================================================

log_message("\nAggregating carbon stocks by AAFC/Canadian Protocol depth intervals...")
...
for (method in METHODS_AVAILABLE) {
  ...
  # Aggregate for each AAFC/Canadian Protocol interval
  interval_stocks <- list()

  for (i in 1:nrow(AAFC/Canadian Protocol_DEPTH_INTERVALS)) {

    depth_mid <- AAFC/Canadian Protocol_DEPTH_INTERVALS$depth_midpoint[i]
    depth_top <- AAFC/Canadian Protocol_DEPTH_INTERVALS$depth_top[i]
    depth_bottom <- AAFC/Canadian Protocol_DEPTH_INTERVALS$depth_bottom[i]
...
for (method in METHODS_AVAILABLE) {
  ...
  if (length(interval_stocks) < nrow(AAFC/Canadian Protocol_DEPTH_INTERVALS)) {
    log_message(sprintf("    Missing some AAFC/Canadian Protocol intervals for %s (have %d, need %d)",
                       method, length(interval_stocks), nrow(AAFC/Canadian Protocol_DEPTH_INTERVALS)), "WARNING")
  }
...
# ============================================================================
# CREATE AAFC/Canadian Protocol CONSERVATIVE ESTIMATES
# ============================================================================

log_message("\nCreating AAFC/Canadian Protocol conservative estimates...")

for (method in METHODS_AVAILABLE) {

  log_message(sprintf("\n  Processing %s AAFC/Canadian Protocol estimates...", toupper(method)))
  ...
  # Save AAFC/Canadian Protocol estimates for this method
  write.csv(vm0033_estimates,
            sprintf("outputs/carbon_stocks/carbon_stocks_conservative_vm0033_%s.csv", method),
            row.names = FALSE)

  log_message(sprintf("  Saved %s AAFC/Canadian Protocol conservative estimates", method))
}
...
# Load all AAFC/Canadian Protocol estimates for visualization
...
if (nrow(all_vm0033_estimates) > 0) {
  ...
  p_methods <- ggplot(all_vm0033_estimates,
    ...
    labs(
      ...
      subtitle = "Red bars show conservative estimates (lower 95% CI) per AAFC/Canadian Protocol",
      x = "Prediction Method",
      y = "Carbon Stock (Mg C/ha)"
    ) +
    theme_minimal() +
...
cat(sprintf("AAFC/Canadian Protocol depth intervals: %d layers\n", nrow(AAFC/Canadian Protocol_DEPTH_INTERVALS)))

if (nrow(all_vm0033_estimates) > 0) {
  cat("\nAAFC/Canadian Protocol Conservative Estimates (0-100 cm):\n")
  cat("------------------------------------------\n")
  ...
  for (i in 1:nrow(all_vm0033_estimates)) {
    ...
    if (!is.na(row$conservative_stock_0_100_Mg_ha)) {
      cat(sprintf("  Conservative Stock: %.2f Mg C/ha (AAFC/Canadian Protocol compliant)\n",
...
cat("  AAFC/Canadian Protocol estimates (by method): carbon_stocks_conservative_vm0033_*.csv\n")

if (length(METHODS_AVAILABLE) > 1) {
  cat("  Method comparison: carbon_stocks_method_comparison.csv\n")
}

cat("\nAAFC/Canadian Protocol/ORRAA Compliance:\n")
cat("----------------------------------------\n")
cat("  ✓ Conservative estimates calculated (lower 95% CI)\n")
cat(sprintf("  ✓ AAFC/Canadian Protocol depth intervals: %d layers (%s)\n",
           nrow(AAFC/Canadian Protocol_DEPTH_INTERVALS),
           paste(sprintf("%.0f-%.0f cm", AAFC/Canadian Protocol_DEPTH_INTERVALS$depth_top,
                        AAFC/Canadian Protocol_DEPTH_INTERVALS$depth_bottom), collapse=", ")))
