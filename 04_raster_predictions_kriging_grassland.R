# ============================================================================
# MODULE 04: GRASSLAND CARBON STRATIFIED KRIGING PREDICTIONS
# ============================================================================
# PURPOSE: Spatial interpolation of SOC using stratified kriging by ecosystem type
# INPUTS:
#   - data_processed/cores_harmonized_spline_grassland.rds (from Module 03)
...
cores_harmonized <- readRDS("data_processed/cores_harmonized_grassland.rds")
...
if (file.exists("data_processed/vm0033_compliance.rds")) {
  ...
  log_message("Loaded AAFC/Canadian Protocol compliance data")
}

# Filter to standard depths only (AAFC/Canadian Protocol midpoints)
cores_standard <- cores_harmonized %>%
  filter(depth_cm %in% STANDARD_DEPTHS)

log_message(sprintf("AAFC/Canadian Protocol target depths: %s cm", paste(STANDARD_DEPTHS, collapse = ", ")))
...
if (dir.exists(gee_strata_dir)) {
  ...
  stratum_file_map <- c(
    "Upper Marsh" = "upper_prairie.tif",
    "Mid Marsh" = "mid_prairie.tif",
    "Lower Marsh" = "lower_prairie.tif",
...
for (depth in STANDARD_DEPTHS) {
  ...
  for (stratum_name in strata) {
    ...
    # Save combined standard error (RECOMMENDED FOR AAFC/Canadian Protocol)
...
# ============================================================================
# AAFC/Canadian Protocol LAYER AGGREGATION: CONCENTRATION TO STOCK
# ============================================================================

log_message("\n=== AAFC/Canadian Protocol Layer Aggregation ===")
...
if (file.exists("data_processed/cores_clean_grassland.rds")) {
  cores_all <- readRDS("data_processed/cores_clean_grassland.rds")
...
# Default BD values if no data available (typical grassland carbon values)
# From literature: mangroves ~0.4-0.6, salt prairiees ~0.3-0.8 g/cm³
...
for (stratum_name in unique(cores_standard$stratum)) {
  ...
  # Process each AAFC/Canadian Protocol layer
  for (i in 1:nrow(AAFC/Canadian Protocol_DEPTH_INTERVALS)) {
    ...
    depth_midpoint <- AAFC/Canadian Protocol_DEPTH_INTERVALS$depth_midpoint[i]
    thickness_cm <- AAFC/Canadian Protocol_DEPTH_INTERVALS$thickness_cm[i]
...
log_message("\nAAFC/Canadian Protocol layer aggregation complete")
...
cat("  3. Run: source('05_raster_predictions_rf_grassland.R')\n\n")
