# ============================================================================
# MODULE 02: GRASSLAND CARBON EXPLORATORY DATA ANALYSIS
# ============================================================================
# PURPOSE: Visualize and explore patterns in grassland carbon data by stratum
# INPUTS: 
...
if (!file.exists("data_processed/cores_clean_grassland.rds")) {
  stop("Cleaned data not found. Run 01_data_prep_grassland.R first.")
}

cores <- readRDS("data_processed/cores_clean_grassland.rds")

log_message(sprintf("Loaded: %d samples from %d cores",
                    nrow(cores),
                    n_distinct(cores$core_id)))

# Load AAFC/Canadian Protocol compliance data if available
...
# Add AAFC/Canadian Protocol compliance status per core if available
...
p_spatial <- ggplot(spatial_data,
  ...
  labs(
    ...
    subtitle = sprintf("n = %d cores across %d strata\nShape = Core Type, Fill = Stratum, Border = AAFC/Canadian Protocol Compliance",
...
# ============================================================================
# PLOT 1B: AAFC/Canadian Protocol COMPLIANCE DASHBOARD
# ============================================================================

if (!is.null(vm0033_compliance)) {
  log_message("Creating AAFC/Canadian Protocol compliance dashboard...")
  ...
  if ("stratum_compliance" %in% names(vm0033_compliance)) {
    ...
    p_sample_size <- ggplot(compliance_data,
      ...
      geom_hline(yintercept = AAFC/Canadian Protocol_MIN_CORES, linetype = "dashed",
                 color = "red", size = 0.8) +
      scale_fill_identity() +
      labs(
        title = "AAFC/Canadian Protocol Sample Size Compliance",
        subtitle = sprintf("Minimum required: %d cores per stratum (red line)\nGray bars show target for 20%% precision",
                          AAFC/Canadian Protocol_MIN_CORES),
        x = "Stratum",
        y = "Number of Cores"
      ) +
      theme(
    ...
    p_precision <- ggplot(compliance_data %>% filter(!is.na(achieved_precision_pct)),
      ...
      geom_hline(yintercept = AAFC/Canadian Protocol_TARGET_PRECISION, linetype = "dashed",
  ...
  log_message("Skipping AAFC/Canadian Protocol compliance dashboard (data not available)", "WARNING")
}
...
cat("\nPlots created:\n")
cat("  Core plots:\n")
cat("    01_spatial_distribution.png - Enhanced with core type shapes & AAFC/Canadian Protocol borders\n")
...
cat("\nKey Enhancements:\n")
cat("  ✓ Added AAFC/Canadian Protocol compliance dashboard\n")
...
cat("  2. Check AAFC/Canadian Protocol compliance and sample size recommendations\n")
cat("  3. Review depth gaps and BD transparency\n")
cat("  4. Run: source('03_depth_harmonization_grassland.R')\n\n")
