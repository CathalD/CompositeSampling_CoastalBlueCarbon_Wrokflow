  ...
  requirements = list(
    list(
      ...
      check_function = function(data) {
        ...
        message <- if (pass) {
          sprintf("All strata have ≥%d samples (min: %d)", VM0026_MIN_CORES, min_samples)
        } else {
  ...
  requirements = list(
    list(
      ...
      check_function = function(data) {
        ...
        message <- if (pass) {
          sprintf("All strata have ≥%d samples", VM0032_MIN_CORES)
        } else {
...
canada_ipcc_criteria <- list(
  ...
  requirements = list(
    ...
    list(
      ...
      threshold = paste0("≤", GRASSLAND_TARGET_PRECISION, "%"),
      check_function = function(data) {
        if (is.null(data$carbon_stocks)) return(list(pass = FALSE, value = NA, message = "No carbon stock data"))
...
cat("\nOutput Files:\n")
cat("-------------\n")
cat("  📄 Comprehensive Report: outputs/reports/comprehensive_grassland_standards_report.html\n")
cat("  📊 Compliance Summary: outputs/reports/grassland_standards_compliance_summary.csv\n")
cat("  📋 Action Plan: outputs/reports/grassland_recommendations_action_plan.csv\n")
